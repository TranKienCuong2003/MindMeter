...
export default function ExpertProfilePage() {
  ...
  if (token) {
    try {
      ...
      user.role = decoded.role || t("roleExpert");
  ...
  React.useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setError(t("fetchExpertError") || "Lỗi tải thông tin chuyên gia");
        setForm({
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true);
    setAlert("");
    // TODO: Gọi API cập nhật thông tin chuyên gia ở đây
    setTimeout(() => {
      setSaving(false);
      setIsEdit(false);
          ...
          <div className="flex flex-col gap-4 mb-4">
            ...
            <div>
              ...
              <div className="text-base text-blue-600 dark:text-blue-300 font-semibold">
                {profile.role === "EXPERT" ? t("roleExpert") : profile.role}
              </div>
            </div>
            {profile.createdAt && (
              <div>
